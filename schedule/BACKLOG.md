...
- [ ] **A-ICONS** amatta 커스텀 SVG 아이콘 재도입(홈/설정 nav·종류 4종·온보딩 블롭) — `docs/design/amatta-icons/`. ⚠️ react-native-svg가 **Expo Go SDK54에선 로드 자체가 크래시**(Circle/Defs "component config 없음"). 현재는 @expo/vector-icons 폴백.
  ...
  - **2026-06-30 빌드 정비 조사 결론**: 패키지는 **완전 정상**. codegenConfig(RNSVGCircle 등) + `src/fabric` 스펙 35개 + `apple/` 네이티브 소스 + `RNSVG.podspec` 모두 존재, autolink 제외 없음, newArch on. 게다가 `react-native` 필드 manifest 버그는 **upstream 15.12.1에서 이미 수정됨**(patch-package "변경 없음" 확인) → `scripts/fix-react-native-svg-manifest.js`는 현재 **no-op**(무해, 방어적, 유지). **고칠 설정 없음.**
  - **원인**: 02:22(UTC) dev 빌드가 RNSVG 네이티브를 안 담은 채 구워짐(stale/EAS 캐시).
  - ~~**풀려면 (코드 변경 X)**: 클린 리빌드면 됨~~ — **이 결론은 틀렸음.** RNSVG를 실제 import하면(A-ICONS·픽업배너) Metro 번들 단계에서 `@react-native/babel-plugin-codegen`이 크래시함. (위 "no-op" 판단도 부정확.)
  - **2026-06-30 진짜 근본원인 + 수정 (검증됨)**: rn-svg의 `react-native` 필드가 **컴파일된 `lib/module/index.js`**(타입 인자 stripped)를 가리켜서, new-arch codegen이 `codegenNativeComponent<NativeProps>(...)`의 타입을 못 읽어 *"Could not find component config"* 로 **번들 자체가 실패**. **수정 = 필드를 타입 있는 소스 `src/index.ts`로** (= `codegenConfig.jsSrcsDir: ./src/fabric`와 일치). `scripts/fix-react-native-svg-manifest.js`를 lib→**src 강제**로 재작성(postinstall, EAS install에도 적용). `npx expo export -p ios` 클린 번들로 검증 완료. A-ICONS·픽업배너는 `feat/svg-icons-pickup-banner`에 적용됨 → 이 브랜치로 **클린 리빌드하면 동작**.
